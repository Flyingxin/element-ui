...
export default {
  ...
  methods: {
    ...
    setPosition(newPosition) {
      ...
      const lengthPerStep = 100 / ((this.max - this.min) / this.step); //步长百分比
      const steps = Math.round(newPosition / lengthPerStep); //步数，并向下取整
      let value =
        steps * lengthPerStep * (this.max - this.min) * 0.01 + this.min;
      value = parseFloat(value.toFixed(this.precision)); // 向下取整
